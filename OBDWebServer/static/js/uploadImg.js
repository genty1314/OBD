 ...
 */
 ...
 UploadImg.prototype = {
    init: function() {
                    ...
                    '<div class="upload-inner">' +
                      '<div class="upload_main">' + 
                        ...
                        '<div class="upload_submit none">' + 
                          ...
                          '<span>&nbsp批量操作:&nbsp</span>'+
                          '<select  id="upload_port_pos_all">'+
                            '<option value ="0">&nbsp端口朝上，0口在左&nbsp</option>'+
                            '<option value ="1">&nbsp端口朝上，0口在右&nbsp</option>'+
                            '<option value ="2">&nbsp端口朝下，0口在左&nbsp</option>'+
                            '<option value ="3">&nbsp端口朝下，0口在右&nbsp</option>'+
                            '<option value ="4">&nbsp端口朝左，0口在上&nbsp</option>'+
                            '<option value ="5">&nbsp端口朝左，0口在下&nbsp</option>'+
                            '<option value ="6">&nbsp端口朝右，0口在上&nbsp</option>'+
                            '<option value ="7">&nbsp端口朝右，0口在下&nbsp</option>'+
                          '</select>'+
                          '<select  id="upload_sort_all" class="upload_sort">'+
                            '<option value ="0">&nbsp顺时针&nbsp</option>'+
                            '<option value ="1">&nbsp逆时针&nbsp</option>'+
                          '</select>'+
                        '</div>' + 
      ...
      // 如果支持拖曳的话
      if (this.isDragDrop) {
        $("#file_"+containerId).html('<span id="drag_'+containerId+'" class="upload_drag_area">将图片拖到此处</span>');
      }
      // 如果需要折叠按钮的话
      if (this.isfold) {
        ...
        for (var a = 0, alen = this.data.length; a < alen; a++) { 
          html += '<li id="uploadList_'+ a +'" class="upload_append_list success">'+
                    ...
                    '<p>'+
                      ...
                      '<span class="upload_port_pos">端口位置:</span>'+
                      '<select  id="upload_port_pos_'+ a +'" class="upload_port_pos" >'+
                        '<option value ="0">&nbsp端口朝上，0口在左&nbsp</option>'+
                        '<option value ="1">&nbsp端口朝上，0口在右&nbsp</option>'+
                        '<option value ="2">&nbsp端口朝下，0口在左&nbsp</option>'+
                        '<option value ="3">&nbsp端口朝下，0口在右&nbsp</option>'+
                        ...
                        '<option value ="7">&nbsp端口朝右，0口在下&nbsp</option>'+
                      '</select>'+
                      '<span class="upload_sort">端口排序:</span>'+
                      '<select  id="upload_sort_'+ a +'" class="upload_sort">'+
                        '<option value ="0">&nbsp顺时针&nbsp</option>'+
                        '<option value ="1">&nbsp逆时针&nbsp</option>'+
                      '</select>'+
                      '<input class="address" id="address_'+a+'" type="text" placeholder="设备地址"></input>'+
                    '</p>'+
                    '<hr width=550%/>'+
                  '</li>';
    ...
    filter: function(files) {
      ...
      for (var i = 0, file; file = files[i]; i++) {
        if (file.size >= 1024000*4) {
          alert('您这张'+ file.name + '图片过大，应小于4M');
        } else {
          arrFiles.push(file);
        }
      }
      ...
      var funAppendImage = function() {
        ...
        if (file) {
          ...
          reader.onload = function(e) {
            ...
            html +='<li id="uploadList_'+ i +'" class="upload_append_list success">'+
                    '<div class="m-layer none" id="m-layer_'+self.containerId+i+'"></div>' + 
                    '<span class="ajax-loader hidden" id="loader_'+self.containerId+i+'"></span>'+
                    '<p>'+
                      ...
                      '<span class="upload_port_pos">端口位置:</span>'+
                      '<select  id="upload_port_pos_'+ i +'" class="upload_port_pos" >'+
                        '<option value ="0">&nbsp端口朝上，0口在左&nbsp</option>'+
                        '<option value ="1">&nbsp端口朝上，0口在右&nbsp</option>'+
                        '<option value ="2">&nbsp端口朝下，0口在左&nbsp</option>'+
                        '<option value ="3">&nbsp端口朝下，0口在右&nbsp</option>'+
                      ...
                      '</select>'+
                      '<span class="upload_sort">端口排序:</span>'+
                      '<select  id="upload_sort_'+ i +'" class="upload_sort">'+
                        '<option value ="0">&nbsp顺时针&nbsp</option>'+
                        '<option value ="1">&nbsp逆时针&nbsp</option>'+
                      '</select>'+ 
                      '<input class="address" id="address_'+i+'" type="text" placeholder="设备地址"></input>'+
                    '</p>'+
                    '<hr width=550%/>'+
                  '</li>';
            i++;
    ...
    uploadFiles: function() {
      ...
      if (self.fileFilter.length) {
        for(var i = 0, ilen = self.fileFilter.length; i < ilen; ) {
          var file = self.fileFilter[i];
          if (!file.successStatus) {
            var formdata = new FormData();
            upp = $("#upload_port_pos_"+i).val();
            us = $("#upload_sort_"+i).val();
            ad = $("#address_"+i).val();

            formdata.append("upload_port_pos",upp);
            formdata.append("upload_sort",us);
